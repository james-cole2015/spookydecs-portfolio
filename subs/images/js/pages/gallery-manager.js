...
function attachGalleryHandlers() {
  ...
  uploadBtn.addEventListener('click', () => {
    // Navigate to upload page with gallery category pre-selected
    const category = getCategoryFromSection(currentSection);
    navigate(`/images/upload?category=${category}`);
  });
...
function getCategoryFromSection(section) {
  return `gallery_${section}`;
}

function getPhotoTypeFromSection(section) {
  const category = getCategoryFromSection(section);
  return IMAGES_CONFIG.CATEGORIES[category]?.photoType;
}
    ...
    if (sortedPhotos.length === 0) {
      grid.innerHTML = `
        <div class="empty-state">
          <p>No photos found</p>
          <button class="btn btn-primary" onclick="document.querySelector('[data-action=upload]').click()">
            Upload Photos
          </button>
        </div>
      `;
...
async function handlePhotoUpdate(photoId, updates) {
  try {
    ...
    await loadGalleryPhotos(); // Reload to show changes
  } catch (error) {
...
async function handlePhotoDelete(photoId) {
  try {
    ...
    await loadGalleryPhotos(); // Reload to remove deleted photo
  } catch (error) {
