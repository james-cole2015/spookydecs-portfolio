...
export default function UnpackedPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const all = await itemsAPI.getUnpacked();
      setItems(all.filter((i) => i.class !== 'Deployment' && i.class !== 'Storage' && i.class_type !== 'Receptacle'));
    } catch (e: any) {
      setError(e?.message ?? 'Failed to load unpacked items');
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading) return <LoadingState label="Loading unpacked items…" />;
  if (error) return <ErrorState message={error} onRetry={load} />;

  return (
    <div>
      <Breadcrumbs crumbs={[{ label: 'Storage', to: '/' }, { label: 'Unpacked Items' }]} />
      <PageHeader title="Unpacked Items" icon={<Search size={26} />} subtitle="Items not yet in storage. Check after packing to catch anything missed." />
      ...
      {filtered.length === 0 ? (
        <EmptyState icon="✅" title="Nothing unpacked" message={items.length === 0 ? 'Every item is in storage. Nice.' : 'No items match the current filters.'} />
      ) : (
