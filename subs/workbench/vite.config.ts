...
// works without duplicating creds into subs/workbench/.
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../..');

// Per-sub Vite config for the workbench React app (#330).
// Mirrors subs/storage/vite.config.ts (#267): the `/devapi` proxy + Cognito
// auto-auth give `npm run dev` here the same ergonomics as the vanilla subs, but
// `npm run build` produces a self-contained bundle to ./dist for the #330
// GitHub Actions deploy (build_command: "npm ci && npm run build:workbench",
// artifact_dir: "dist").
