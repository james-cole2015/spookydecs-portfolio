/**
 * Finance configuration & constants — typed port of the vanilla finance-config.js.
 *
 * Business logic (validators, filters, sorters, formatters, ID generation) is
 * preserved verbatim from the original; only TypeScript types were added. (#333)
 */
...
export const COST_TYPES: SelectOption[] = [
...
export const CATEGORIES_BY_COST_TYPE: Record<string, SelectOption[]> = {
  acquisition: [
  ...
  supply_purchase: [{ value: 'consumables', label: 'Consumables' }],
...
export interface RelatedIdConfig {
  field: string;
  label: string;
...
export const RELATED_ID_CONFIG: Record<string, RelatedIdConfig | null> = {
  acquisition: {
    field: 'related_item_id',
    label: 'Related Item',
    required: (category: string) => ['decoration', 'light', 'accessory'].includes(category),
    endpoint: '/items',
...
export function getRelatedIdConfig(costType: string): RelatedIdConfig | null {
  return RELATED_ID_CONFIG[costType];
}

// Check if related ID is required for given cost type and category
export function isRelatedIdRequired(costType: string, category: string): boolean {
...
export function validateCostRecord(record: Record<string, any>): ValidationResult {
...
export function getCategoriesForCostType(costType: string): SelectOption[] {
  return CATEGORIES_BY_COST_TYPE[costType] || CATEGORIES_BY_COST_TYPE.other;
}

// Calculate value based on is_gift flag
export function calculateValue(isGift: boolean, totalCost: number | string, _tax = 0): number {
  if (isGift) {
    // For gifts, value should be manually entered, not calculated
    return 0;
  }
  return parseFloat(totalCost as string) || 0;
}
...
// ID Generation
export function generateCostId(existingCosts: CostRecord[] = []): string {
...
export interface CostFilters {
  ...
  category?: string;
  vendor?: string;
  is_gift?: boolean | string;
  startDate?: string;
  endDate?: string;
}

// Filter Functions
export function filterCosts(costs: CostRecord[], filters: CostFilters): CostRecord[] {
  return costs.filter((cost) => {
    ...
    if (filters.startDate) {
      if (new Date(cost.cost_date as string) < new Date(filters.startDate)) return false;
    }

    if (filters.endDate) {
      if (new Date(cost.cost_date as string) > new Date(filters.endDate)) return false;
    }
...
// Sort Functions
export function sortCosts(costs: CostRecord[], sortBy: string, sortOrder = 'desc'): CostRecord[] {
...
export function calculateTotalCost(unitCost: number | string, quantity: number | string): string {
  const unit = parseFloat(unitCost as string) || 0;
  const qty = parseFloat(quantity as string) || 1;
