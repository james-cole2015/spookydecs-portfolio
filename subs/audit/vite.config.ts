...
// works without duplicating creds into subs/audit/.
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../..');

// Per-sub Vite config for the audit React app (#341).
// Mirrors subs/workbench/vite.config.ts (#330) / subs/storage (#267): the
// `/devapi` proxy + Cognito auto-auth give `npm run dev` here the same
// ergonomics as the vanilla subs, but `npm run build` produces a self-contained
// bundle to ./dist for the GitHub Actions deploy (build_command:
// "npm ci && npm run build:audit", artifact_dir: "dist").
