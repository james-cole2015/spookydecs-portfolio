...
// works without duplicating creds into subs/ideas/.
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../..');

// Per-sub Vite config for the ideas React app (#334), mirroring subs/storage/.
// Replicates the repo-root dev ergonomics (the `/devapi` proxy + Cognito
// auto-auth) so `npm run dev` here behaves like the vanilla subs, but builds a
// self-contained bundle to ./dist for the #269 GitHub Actions deploy.
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    // local configs-spookydecs repo (#357). No-op in `vite build`.
